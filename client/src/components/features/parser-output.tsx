import { AlertCircle, AlertTriangle, Info, X, Plus, Eye, EyeOff } from 'lucide-react';
                ...
                <table className="w-full text-[11px] border-collapse">
                  ...
                  <tbody>
                    {filteredRegistry.map((record, idx) => {
                      ...
                      const digitalReads = ops.filter(u => u.operation.includes('digitalRead'));
                      const digitalWrites = ops.filter(u => u.operation.includes('digitalWrite'));
                      const analogReads = ops.filter(u => u.operation.includes('analogRead'));
                      const analogWrites = ops.filter(u => u.operation.includes('analogWrite'));
                      const pinModes = ops.filter(u => u.operation.includes('pinMode')).map(u => {
                        const match = u.operation.match(/pinMode:(\d+)/);
                        const mode = match ? parseInt(match[1]) : -1;
                        return mode === 0 ? 'INPUT' : mode === 1 ? 'OUTPUT' : mode === 2 ? 'INPUT_PULLUP' : 'UNKNOWN';
                      ...
                      return (
                        <tr key={record.pin} className={`border-b border-muted-foreground/10 h-7 ${idx % 2 === 0 ? 'bg-background' : 'bg-muted/20'}`}>
                          {/* Pin Column */}
                          <td className="px-2 py-1 text-right font-mono font-semibold text-cyan-400">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              {(() => {
                                ...
                                return !Number.isNaN(n) && PWM_PINS.includes(n) ? <span className="text-yellow-400">~</span> : null;
                              })()}
                          ...
                          {/* pinMode Column */}
                          <td className={clsx("px-2 py-1 text-center", hasMultipleModes && "border-2 border-red-500")}>
                            {pinModes.length > 0 ? (
                              <div className="space-y-0.5 text-center">
                                {uniqueModes.map((mode, i) => {
                                  const count = pinModes.filter(m => m === mode).length;
                                  const modeColor = mode === 'INPUT' ? 'text-blue-400' : mode === 'OUTPUT' ? 'text-orange-400' : 'text-green-400';
                                  return (
                              ...
                              <div className="text-center">
                                <span className={
                                  record.pinMode === 0 ? 'text-blue-400' :
                                  record.pinMode === 1 ? 'text-orange-400' :
                                  'text-green-400'
                                }>
                                  {record.pinMode === 0 ? 'INPUT' :
                                   record.pinMode === 1 ? 'OUTPUT' :
                                   'INPUT_PULLUP'}
                                </span>
                              </div>
