...
export function CompilationOutput({ output, onClear, isSuccess = false, showSuccessMessage = true, hideHeader = false }: CompilationOutputProps) {
  return (
    <div className="h-full flex flex-col border-b border-border" data-testid="compilation-output">
        ...
        <div className="flex items-center w-full min-w-0 overflow-hidden whitespace-nowrap gap-2">
          <div className="flex items-center space-x-2 flex-shrink-0">
            {isSuccess && showSuccessMessage ? (
              <CheckCircle2 className="text-green-500 opacity-95 h-5 w-5" strokeWidth={1.67} />
            ) : (
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        <div className="console-output p-3 font-mono whitespace-pre-wrap" data-testid="compilation-text">
          {output || (
