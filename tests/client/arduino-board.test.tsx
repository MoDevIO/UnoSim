...
describe('ArduinoBoard - Pin Frame Visibility', () => {
  ...
  it('should display yellow frame for pin 0 when configured as INPUT', async () => {
    ...
    await waitFor(() => {
      const overlayDiv = document.querySelector('[data-overlay="true"]');
    ...
    // Check that pin-0-frame exists and is visible
  ...
  it('should display yellow frames for pins 0-6 when all configured as INPUT', async () => {
    ...
    // Check all pins 0-6 have visible frames
    for (let pin = 0; pin < 7; pin++) {
      ...
      expect(frame.style.filter, `pin ${pin} frame should have glow`).toContain('drop-shadow');
    }
  });
