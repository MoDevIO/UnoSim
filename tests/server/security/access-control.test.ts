...
describe("parseTrustConfig", () => {
  it("uses local mode outside production", () => {
    expect(parseTrustConfig({ NODE_ENV: "development" })).toEqual({ mode: "local" });
  });
  ...
  it("rejects incomplete and invalid gateway configuration", () => {
    expect(() => parseTrustConfig({ UNOSIM_TRUST_MODE: "public" })).toThrow(/must be either/);
    expect(() => parseTrustConfig({ UNOSIM_TRUST_MODE: "gateway" })).toThrow(/at least 32/);
...
describe("authorizeHeaders", () => {
  it("provides a fixed identity in local mode and ignores spoofed identity headers", () => {
    expect(authorizeHeaders({ "x-unosim-subject": "attacker" }, { mode: "local" })).toEqual({
      allowed: true,
  ...
  it("rejects missing or incorrect gateway credentials", () => {
    expect(authorizeHeaders({}, gatewayTrust)).toEqual({ allowed: false, status: 401 });
    expect(authorizeHeaders({
      "x-unosim-gateway-secret": `${SECRET}-wrong`,
      "x-unosim-subject": "student-1",
      "x-unosim-roles": "user",
    }, gatewayTrust)).toEqual({ allowed: false, status: 401 });
  });
  ...
  it("rejects invalid subjects and unauthorized roles", () => {
    ...
    }, gatewayTrust)).toEqual({ allowed: false, status: 403 });
  });
...
describe("HTTP authorization middleware", () => {
  it("protects a route in gateway mode", async () => {
    const app = express();
    app.get("/protected", createUserAuthorizationMiddleware(gatewayTrust), (_req, res) => {
