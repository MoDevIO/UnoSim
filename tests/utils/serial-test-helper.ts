...
export async function runSketchWithOutput(
  ...
  const result = await new Promise<{
    ...
    const runPromise = runner.runSketch({
      ...
      onOutput: (chunk: string | Buffer) => {
        ...
        if (line.includes("--- Simulation timeout")) {
          ...
          console.error(
            "[runSketchWithOutput] Simulation timeout output received:",
            line,
          );
      ...
      onError: (error: string) => {
        ...
        stopAndResolve({ outputs, success: false, error });
      },
      ...
      onCompileError: (error: string) => {
        ...
        // onCompileError
        stopAndResolve({ outputs, success: false, error: `Compile: ${error}` });
      },
      onCompileSuccess: () => {
      ...
      onIORegistry: (registry, baudrate) => {
        log("onIORegistry:", {
          registryLength: Object.keys(registry).length,
          baudrate,
        });
    ...
    if (resolved) clearTimeout(fallbackTimer);
  });
