...
await runner.runSketch(
  ...
  (registry) => {
    ...
    console.log('  Pins with operations:', registry.filter(p => p.usedAt && p.usedAt.length > 0).length);
  }
);
...
setTimeout(() => {
  try {
    const newestFile = execSync('ls -t temp/io-registry-*.json 2>/dev/null | head -1').toString().trim();
    if (newestFile) {
      ...
      console.log('Defined pins:', registry.filter(p => p.defined).map(p => p.pin).join(', '));
      
      // Zeige Details der definierten Pins
      console.log('\n=== Pin Details ===');
      registry.filter(p => p.defined).forEach(pin => {
        const ops = pin.usedAt ? pin.usedAt.map(u => u.operation).join(', ') : 'none';
        console.log(`  Pin ${pin.pin}: defined at line ${pin.definedAt?.line || 0}, operations: ${ops}`);
      });
    } else {
