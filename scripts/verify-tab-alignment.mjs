...
async function verifyTabAlignment() {
  ...
  for (const scale of scales) {
    ...
    await page.evaluate((s) => {
      ...
      window.dispatchEvent(new CustomEvent('uiFontScaleChange', { detail: { value: s } }));
      document.dispatchEvent(new CustomEvent('uiFontScaleChange', { detail: { value: s } }));
    }, scale);
    ...
    const measurement = await page.evaluate(() => {
      const tabHeader = document.querySelector('[data-testid="output-tabs-header"]') 
        || document.querySelector('.flex.items-center.justify-between.px-2.h-\\[var\\(--ui-button-height\\)\\].bg-muted.border-b');
    ...
    const status = measurement.gap !== undefined && Math.abs(measurement.gap) <= 1 ? '✓' : '✗';
  ...
  const allPassed = results.every(r => r.gap !== undefined && Math.abs(r.gap) <= 1);
