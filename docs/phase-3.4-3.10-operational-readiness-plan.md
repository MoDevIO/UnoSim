...
| 3.6 | Coverage-Hotspots | `partial` | WS 61,90 % Lines; Cache-Manager 100 % Lines; Local Compiler nach Kernpfad-Tests 84,45 % Lines / 84,31 % Statements / 65,38 % Branches; Routes 28,57 %. | Weitere aktuelle Hotspots inkrementell testen. |
...
| 3.6 | Hotspots sind benannt; WebSocket-, Cache- und Local-Compiler-Kernpfade sind behavior-orientiert getestet; Coverage-Report und `npm run test:coverage` existieren. | Weitere aktuelle Hotspots testen, bis alle Hotspots >60 % und kritische Hotspots >80 % erreichen; jede Abweichung muss explizit begründet werden. |
...
1. **3.6 Coverage-Hotspots** mit kleinen, behavior-orientierten Test-Inkrementen fortsetzen; Cache-Manager und Local-Compiler-Kernpfade sind abgeschlossen, nächster Kandidat ist `server/routes.ts`.
