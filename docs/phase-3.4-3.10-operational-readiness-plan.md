...
| Voraussetzungen | Docker-Daemon verfügbar; `unosim-sandbox:latest` gebaut; definierte Hostklasse; keine parallele Last auf demselben Docker-Daemon; Release-Aufruf mit `REQUIRE_RELEASE_GATE=1`. |
...
| Feld | Inhalt |
| --- | --- |
| Ziel | Ein verbindliches Release-Gate definieren: Typecheck, Unit/Coverage, relevante Integration, Docker, E2E, Build, SonarQube und Security-Audit mit klaren Abbruchregeln. |
...
| Relevante Tests/Gates | `REQUIRE_RELEASE_GATE=1 ./run-tests.sh` (zentraler Release-Aufruf); Einzelgates bleiben für Diagnose verfügbar. |
