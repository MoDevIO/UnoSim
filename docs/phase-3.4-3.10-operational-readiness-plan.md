...
| 3.6 | Coverage-Hotspots | `partial` | WS 61,90 % Lines; Cache-Manager 100 % Lines; Local Compiler 84,45 % Lines; `server/routes.ts` 75,23 % Lines / 74,10 % Statements / 46,42 % Branches; `useSimulatorFileSystem.ts` 94,64 % Lines; `output-panel.tsx` 68,96 % Lines; `examples-menu.tsx` 69,23 % Lines. | Weitere aktuelle Client-/Sandbox-Hotspots inkrementell testen. |
...
| 3.6 | Hotspots sind benannt; WebSocket-, Cache-, Local-Compiler-, zentrale REST- und die drei priorisierten Client-Pfade sind behavior-orientiert getestet; Coverage-Report und `npm run test:coverage` existieren. | Verbleibende Client-/Sandbox-Hotspots testen, bis alle Hotspots >60 % und kritische Hotspots >80 % erreichen; jede Abweichung muss explizit begründet werden. |
...
1. **3.6 Coverage-Hotspots** mit kleinen, behavior-orientierten Test-Inkrementen fortsetzen; Cache-Manager, Local-Compiler-Kernpfade und REST-Kernrouten sind abgeschlossen, nächste Kandidaten liegen in verbleibenden Client-/Sandbox-Hotspots.
